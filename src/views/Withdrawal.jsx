  ...
  const sendInput = (val) => {
    ...
    if (isNaN(val) || val == 0 || val == null) {
      alt({
        type: DropdownAlertType.Error,
        title: strings.validation_error,
        message: strings.please_enter_valid_amount,
      });
      ...
      alt({
        type: DropdownAlertType.Error,
        title: strings.validation_error,
        message: strings.your_maximum_wallet_balance_is + w_amount,
      });
  ...
  const validate_amount = () => {
    if (w_amount > 0) {
      ...
      alt({
        type: DropdownAlertType.Error,
        title: strings.validation_error,
        message: strings.your_balance_is_low,
      });
      ...
      <ScrollView>
        <View style={[styles.header]}>
          ...
          <View activeOpacity={1} style={{ width: '85%', alignItems: 'flex-start', justifyContent: 'center' }}>
            <Text numberOfLines={1} ellipsizeMode='tail' style={{ color: colors.theme_fg_three, fontSize: f_xl, fontFamily: bold }}>{strings.withdrawal}</Text>
          </View>
        </View>
        ...
        >
          <View style={{ backgroundColor: colors.theme_bg_three, width: '100%', padding: 20, alignItems: 'center', justifyContent: 'center', borderRadius: 10, flexDirection: 'row' }}>
            ...
            <View style={{ width: '75%', alignItems: 'flex-start', justifyContent: 'center' }}>
              <Text numberOfLines={1} style={{ color: colors.text_grey, fontSize: f_s, fontFamily: normal }}>{strings.current_balance}</Text>
          ...
          <View style={{ backgroundColor: colors.theme_bg_three, width: '100%', padding: 20, justifyContent: 'center', borderRadius: 10 }}>
            <Text numberOfLines={1} style={{ color: colors.text_grey, fontSize: f_s, fontFamily: normal }}>{strings.withdrawal_histories}</Text>
            <View style={{ margin: 10 }} />
            {on_load == 1 &&
            <FlatList
              data={data}
