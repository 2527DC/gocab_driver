...
import { normal, bold, regular, screenHeight, screenWidth, dashboard, api_url, change_online_status, LATITUDE_DELTA, 
  LONGITUDE_DELTA, f_s, f_tiny, f_xs, get_heatmap_coordinates} from '../config/Constants.js';
...
import { changeLocation } from '../actions/ChangeLocationActions.js';
import { initialLat, initialLng, initialRegion } from '../actions/BookingActions.js';
  ...
  const get_location = async(vt,sy) =>{
    const granted = await PermissionsAndroid.request(
      PermissionsAndroid.PERMISSIONS.ACCESS_FINE_LOCATION, {
          ...
          message: {app_name}+ strings.needs_to_access_your_location_in_background_for_get_nearest_trips_show_live_location_to_customers_that_will_be_always_in_use
          }
      );
      if(granted && vt != 0) {
    ...
    <View style={styles.container}>
      ...
      <View style={{ position:'absolute', top:90, width:'100%' }}>
        ...
        <TouchableOpacity activeOpacity={1} onPress={navigate_wallet.bind(this)} style={{ flexDirection:'row', backgroundColor:colors.error_background, borderRadius:10, alignItems:'center', justifyContent:'center', padding:10, width:'90%', marginLeft:'5%'}}>
          ...
          <View style={{ margin:5 }} />
          <Text style={{ fontFamily:regular, fontSize:f_xs, color:colors.error}}>{strings.your_wallet_balance_is_low_please_recharge_immediately}</Text>
        </TouchableOpacity>
        }
      </View>      
    </View>
